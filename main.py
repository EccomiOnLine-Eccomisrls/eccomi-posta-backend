...
def telegramma_submit_preview_html(pratica_id: str):
    """
    Anteprima grafica del Submit Telegramma.
    ...
    try:
        # Utilizza l'anteprima tecnica già funzionante
        preview = telegramma_submit_preview(pratica_id)
        ...
        if not isinstance(preview, dict):
            return HTMLResponse(
                ...
                <body style="font-family:Arial;padding:30px;">
                    ...
                    <p>La risposta tecnica non è valida.</p>
                    <a href="/dashboard/pratiche">
                        ← Torna alla dashboard
                    </a>
                </body>
        ...
        mittente = preview.get("mittente") or {}
        destinatario = preview.get("destinatario") or {}

        mitt_anagrafica = (
            preview.get("mittente_anagrafica_poste")
            or {}
        )

        dest_anagrafica = (
            preview.get("destinatario_anagrafica_poste")
            or {}
        )
        ...
        def esc(value):
            valore = "-" if value in [None, ""] else value
        ...
        testo_telegramma = pratica.get("testo") or ""
        ...
        mitt_tipo = (
            "AZIENDA / ENTE"
            if mitt_anagrafica.get("is_azienda")
            else "PERSONA FISICA"
        )
        ...
        mitt_nome_completo = (
            mitt_anagrafica.get("ragione_sociale")
            or " ".join(
                parte
                for parte in [
                    str(
...
<head>
    ...
    <title>Anteprima XML Telegramma {esc(ordine)}</title>
...
<div class="page">
    ...
    <div class="card">
        ...
        <div class="grid">
            ...
            <div class="field">
                ...
                <span class="value">{esc(cliente_email)}</span>
            </div>

            <div class="field">
                <span class="label">Numero parole</span>
                <span class="value">{esc(parole)}</span>
            </div>

            <div class="field">
                <span class="label">ID anteprima</span>
                <span class="value">
                    {esc(preview.get("id_request"))}
                </span>
            </div>
        </div>

        <div class="safe-box">
            Invio reale a Poste: NO. Questa pagina genera
            esclusivamente l'anteprima XML del Telegramma.
        </div>
    </div>
    ...
    <div class="grid">
        ...
        <div class="card">
            <div style="
                ...
                flex-wrap:wrap;
            ">
                <h2 style="margin:0;">Mittente</h2>

                <span class="badge {mitt_badge}">
                    {esc(mitt_tipo)}
                </span>
            </div>

            <div class="address">
                <strong>{esc(mitt_nome_completo)}</strong>
                <br>
                {esc(mittente.get("indirizzo"))}
                <br>
                {esc(mittente.get("cap"))}
                {esc(mittente.get("comune"))}
                ({esc(mittente.get("provincia"))})
                <br>
                Telefono: {esc(mittente.get("telefono"))}
            </div>
            ...
            <div class="grid">
                ...
                <div class="field">
                    ...
                    <span class="value">
                        {esc(mitt_anagrafica.get("cognome"))}
                    </span>
                </div>
        ...
        <div class="card">
            <div style="
                ...
                flex-wrap:wrap;
            ">
                <h2 style="margin:0;">Destinatario</h2>

                <span class="badge {dest_badge}">
                    {esc(dest_tipo)}
                </span>
            </div>

            <div class="address">
                <strong>{esc(dest_nome_completo)}</strong>
                <br>
                {esc(destinatario.get("indirizzo"))}
                <br>
                {esc(destinatario.get("cap"))}
                {esc(destinatario.get("comune"))}
                ({esc(destinatario.get("provincia"))})
                <br>
                Telefono: {esc(destinatario.get("telefono"))}
            </div>
            ...
            <div class="grid">
                ...
                <div class="field">
                    <span class="label">Cognome</span>
                    <span class="value">
                        {esc(dest_anagrafica.get("cognome"))}
                    </span>
                </div>
    ...
    <div class="card">
        ...
        <div class="telegram-text">{esc_pre(testo_telegramma)}</div>
    </div>
    ...
    <div class="card">
        ...
        <div class="grid">
            <div>
                <h3>Mittente</h3>
                <div class="raw-box">{esc_pre(mittente_json)}</div>
            </div>

            <div>
                <h3>Destinatario</h3>
                <div class="raw-box">{esc_pre(destinatario_json)}</div>
            </div>
        </div>
    </div>

    <div class="card">
        <h2>XML Telegramma generato</h2>

        <div class="warning">
            Controllare Nome, Cognome, RagioneSociale,
            CAP, città e indirizzo prima di riattivare
            l'automazione Telegramma.
        </div>

        <pre class="xml">{esc_pre(xml_preview)}</pre>
    </div>
    ...
    try:
        ...
        if poste_invio_mode == "disabled":
            return {
                ...
                "order_id": order_id,
                "error": (
                    "Invio Poste produzione disabilitato "
                    "da POSTE_INVIO_MODE."
                )
            }
        ...
        if stato_ordine not in stati_lavorabili:
            return {
                "success": False,
                "blocked": True,
                "step": "STATO_ORDINE_NON_LAVORABILE",
                "error": (
                    "Invio Poste bloccato: ordine in stato "
                    f"{stato_ordine} non lavorabile"
                ),
                "stato": stato_ordine,
                "order_id": order_id,
...
def process_poste_order(order_id: str):
    """
    Endpoint pubblico/manuale.
    AUTO_INVIO_IN_CORSO non è autorizzato.
    Un secondo processo non può entrare nell'ordine bloccato.
    """
    return _process_poste_order_core(
        order_id=order_id,
        allow_auto_lock=False
    )
        ...
        if tipo_servizio == "TELEGRAMMA":
            return RedirectResponse(
                url=(
                    ...
                    f"submit-preview-html/{pratica_id}"
                ),
                status_code=302
            )
...
def dashboard_invia_diretto_poste(pratica_id: str):
    """
    Invio diretto, automatico e controllato a Poste.
    Sicurezze:
    - abilitazione tramite variabili Render
    - solo servizio RACCOMANDATA
    - solo pratica RICEVUTO_PAGATO
    - blocco pratiche già inviate
    - blocco id_richiesta o numero raccomandata già presenti
    - lock atomico su poste_h2h_orders
    - AUTO_INVIO_IN_CORSO non è normalmente lavorabile
    - timeline completa degli eventi
    - conteggio tentativi
    """
    h2h_order_id = None
    pratica = None
    nuovo_tentativo = None
    try:
        # =====================================================
        # 1. RECUPERO PRATICA
        # =====================================================
        ...
        if not pratica:
            ...
            return RedirectResponse(
                url=(
                    "/dashboard/pratiche"
                    "?errore=pratica_non_trovata"
                ),
                status_code=302
            )
        stato_pratica_iniziale = str(
            pratica.get("stato") or ""
        ).strip().upper()
        # =====================================================
        ...
        if (
            poste_invio_mode != "auto"
            or not direct_enabled
        ):
            messaggio_blocco = (
                "Invio diretto Poste bloccato dalla "
                "configurazione. "
        ...
        if not h2h_order_id:
            errore = (
                "Impossibile trovare l'ordine H2H "
        ...
        # AUTO_INVIO_IN_CORSO non deve essere inserito qui.
        ...
        if not lock_res.data:
            motivo = (
                "Lock non acquisito. L'ordine potrebbe "
                "essere già in elaborazione da un altro "
                "processo."
            )
            print(
            ...
            return RedirectResponse(
                url=(
                    "/dashboard/pratiche"
                    "?errore=ordine_in_elaborazione"
                ),
                status_code=302
            )
        # Da questo punto il lock è acquisito.
        # Nessun secondo processo può lavorare questo ordine.
        # =====================================================
        # 6. INCREMENTO TENTATIVO E STATO PRATICA
        # =====================================================
        try:
            tentativi_precedenti = int(
                pratica.get("auto_invio_tentativi")
                or 0
            )
        except Exception:
            tentativi_precedenti = 0
        ...
        registra_evento_pratica(
            ...
            messaggio=(
                "Avviato invio automatico Raccomandata "
                "a Poste. "
        ...
        # =====================================================
        # 7. PREPARAZIONE E PREZZATURA POSTE
        # =====================================================
        ...
        registra_evento_pratica(
            ...
            messaggio=(
                "Preparazione e prezzatura Poste "
                "completate. Avvio conferma definitiva."
            ),
            source="dashboard_invia_diretto_poste",
            payload={
                "h2h_order_id": h2h_order_id,
        ...
        if (
            ...
            registra_evento_pratica(
                ...
                messaggio=(
                    "Errore durante la conferma definitiva "
                    "Poste. Non ripetere automaticamente senza "
                    f"verifica. Dettaglio: {errore}"
                ),
                source="dashboard_invia_diretto_poste",
                payload={
                    "h2h_order_id": h2h_order_id,
        ...
        registra_evento_pratica(
            ...
            stato_nuovo="INVIATO_POSTE",
            messaggio=(
                "Raccomandata inviata e confermata "
                "definitivamente presso Poste Italiane."
            ),
            source="dashboard_invia_diretto_poste",
            payload={
                "h2h_order_id": h2h_order_id,
        ...
        # =====================================================
        # RECUPERO SICURO DOPO ECCEZIONE
        # =====================================================
        if h2h_order_id:
            try:
                (
...
def dashboard_raccomandata_test_auto(pratica_id: str):
    ...
    Idempotente:
    - se esiste già una richiesta TEST finalizzata, non crea nuove richieste
    - se esiste una richiesta TEST pendente, riprova solo PreConferma
    - se non esiste nulla, esegue Invio + Valorizza + PreConferma + Stato/Documento
    NON usa produzione.
    NON invia email.
    ...
    try:
        ...
        # =====================================================
        # 1. Leggo eventuale stato TEST già presente
        # =====================================================
        poste_response_existing = pratica.get("poste_response") or {}
        if isinstance(poste_response_existing, str):
            try:
                poste_response_existing = json.loads(poste_response_existing)
            except Exception:
                poste_response_existing = {}
        if not isinstance(poste_response_existing, dict):
            poste_response_existing = {}
        raccomandata_test_existing = poste_response_existing.get("raccomandata_test") or {}
        if not isinstance(raccomandata_test_existing, dict):
            raccomandata_test_existing = {}
        preconferma_existing = raccomandata_test_existing.get("preconferma_test") or {}
        if not isinstance(preconferma_existing, dict):
            preconferma_existing = {}
        existing_id_richiesta_test = str(
            raccomandata_test_existing.get("id_richiesta_test")
            or preconferma_existing.get("id_richiesta_test")
            or ""
        ).strip()
        existing_guid_utente_test = str(
            raccomandata_test_existing.get("guid_utente_test")
            or preconferma_existing.get("guid_utente_test")
            or ""
        ).strip()
        existing_id_ordine_poste_test = str(
            raccomandata_test_existing.get("id_ordine_poste_test")
            or preconferma_existing.get("id_ordine_poste_test")
            or ""
        ).strip()
        existing_numero_test = str(
            raccomandata_test_existing.get("numero_raccomandata_test")
            or preconferma_existing.get("numero_raccomandata_test")
            or ""
        ).strip()
        existing_id_ricevuta_test = str(
            raccomandata_test_existing.get("id_ricevuta_test")
            or preconferma_existing.get("id_ricevuta_test")
            or ""
        ).strip()
        existing_costo_test = (
            raccomandata_test_existing.get("costo_test_finale")
            if raccomandata_test_existing.get("costo_test_finale") is not None
            else preconferma_existing.get("costo_test_finale")
        )
        already_finalized_test = (
            bool(existing_numero_test)
            and existing_id_ordine_poste_test not in ["", "None", "none", "null"]
            and existing_costo_test is not None
        )
        # =====================================================
        # 2. Pipeline idempotente
        # =====================================================
        # Caso A: TEST già finalizzato.
        # Non crea una nuova richiesta.
        if already_finalized_test:
            results["calcola"] = {
                "skipped": True,
                "reason": "Richiesta TEST già finalizzata. Nessun nuovo Invio/Valorizza."
            }
            finalizza_result = {
                "success": True,
                "step": "RACCOMANDATA_TEST_PRECONFERMA_GIA_PRESENTE",
                "ambiente": "TEST",
                "pratica_id": pratica_id,
                "order_name": pratica.get("shopify_order_name") or pratica.get("order_name"),
                "id_richiesta_test": existing_id_richiesta_test,
                "guid_utente_test": existing_guid_utente_test,
                "id_ordine_poste_test": existing_id_ordine_poste_test,
                "numero_raccomandata_test": existing_numero_test,
                "id_ricevuta_test": existing_id_ricevuta_test,
                "costo_test_finale": existing_costo_test,
                "message": "PreConferma TEST già presente. Nessuna nuova richiesta creata."
            }
            results["finalizza"] = finalizza_result
        # Caso B: esiste già una richiesta TEST ma non è pronta.
        # Non rifà calcola, riprova solo PreConferma.
        elif existing_id_richiesta_test:
            results["calcola"] = {
                "skipped": True,
                "reason": "Esiste già id_richiesta_test. Non creo una nuova richiesta TEST.",
                "id_richiesta_test": existing_id_richiesta_test,
                "guid_utente_test": existing_guid_utente_test
            }
            finalizza_result = dashboard_raccomandata_test_finalizza(pratica_id)
            results["finalizza"] = finalizza_result
        # Caso C: non esiste richiesta TEST.
        # Parte da zero: Invio + Valorizza + PreConferma.
        else:
        ...
        if (
            ...
            return {
                ...
                "message": "La pipeline TEST ha una richiesta già avviata, ma Poste non ha ancora restituito numero raccomandata/costo. Rilancia lo stesso endpoint auto tra qualche minuto: non verrà creata una nuova richiesta."
            }
        # =====================================================
        ...
        if not isinstance(raccomandata_test, dict):
            raccomandata_test = {}
        calcola_entry = results.get("calcola") or {}
        finalizza_entry = results.get("finalizza") or {}
        stato_entry = results.get("stato_documento") or {}
        raccomandata_test["auto_pipeline"] = {
            ...
            "idempotent": True,
            "results_summary": {
                "calcola_success": bool(calcola_entry.get("success")) if isinstance(calcola_entry, dict) else False,
                "calcola_skipped": bool(calcola_entry.get("skipped")) if isinstance(calcola_entry, dict) else False,
                "finalizza_success": bool(finalizza_entry.get("success")) if isinstance(finalizza_entry, dict) else False,
                "stato_documento_success": bool(stato_entry.get("success")) if isinstance(stato_entry, dict) else False,
        ...
        return ecx_clean_for_supabase({
            ...
            "documento_size_bytes": stato_result.get("documento_size_bytes"),
            "results": results,
            "message": "Pipeline automatica Raccomandata TEST completata in modo idempotente. Produzione non toccata."
        })
    except Exception as e:
        return ecx_clean_for_supabase({
            "success": False,
